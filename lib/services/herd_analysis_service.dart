// lib/services/herd_analysis_service.dart
...
// Classe auxiliar para representar um ciclo de lactação
class LactationCycle {
  final DateTime startDate;
  final DateTime? endDate;
  final List<MilkRecord> records;

  LactationCycle({required this.startDate, this.endDate, required this.records});

  int get lengthInDays {
    final end = endDate ?? DateTime.now();
    // Garante que o período de lactação não seja maior que o padrão de 305 dias para o cálculo da média
    final effectiveEnd = end.difference(startDate).inDays > 305 ? startDate.add(const Duration(days: 305)) : end;
...
class HerdAnalysisService {
  ...
  Map<String, double?> analyze(DateTimeRange period) {
    return {
      ...
      'averageDailyMilkProduction': _calculateAverageDailyMilkProduction(period),
    };
  }
  
  // --- ÍNDICES DE PESO E CORTE ---
  double? _calculateAverageAdgBirthToWeaning(DateTimeRange period) {
    final weanedCalves = herd.where((a) =>
        a.isDesmamado &&
        a.dataDesmame != null &&
        !a.dataDesmame!.isBefore(period.start) &&
        !a.dataDesmame!.isAfter(period.end)).toList();
        
    if (weanedCalves.isEmpty) return null;

    final adgValues = <double>[];
    for (final calf in weanedCalves) {
      final birthWeightRecord = calf.historicoPeso.sortedBy((r) => r.date.difference(calf.dataNascimento).abs()).firstOrNull;
      final weanWeightRecord = calf.historicoPeso.sortedBy((r) => r.date.difference(calf.dataDesmame!).abs()).firstOrNull;
  ...
  double? _calculateAverageDailyMilkProduction(DateTimeRange period) {
    ...
    final dailyAverages = lactationCycles.map((lc) => lc.averageDailyProduction).whereNotNull().toList();
  ...
  List<LactationCycle> _getAllLactationCycles(DateTimeRange period) {
    ...
    for (final female in females) {
      final calvings = female.historicoReprodutivo.where((e) => e.eventType == 'Parto').sortedBy((e) => e.date).toList();
      ...
      for (int i = 0; i < calvings.length; i++) {
        ...
        if (!startDate.isAfter(period.end)) {
           final milkInCycle = female.historicoLeite.where((r) => !r.date.isBefore(startDate) && (endDate == null || r.date.isBefore(endDate))).toList();
           if (milkInCycle.isNotEmpty) {
             cycles.add(LactationCycle(startDate: startDate, endDate: endDate, records: milkInCycle));
           }
        }
      }
    }
  ...
  double? _calculateMortalityRate(DateTimeRange period) {
    final animalsAtStart = herd.where((a) => a.dataNascimento.isBefore(period.start)).toList();
    if (animalsAtStart.isEmpty) return 0.0;
    final deadAnimals = animalsAtStart.where((a) => a.status == AnimalStatus.morto && a.dataSaida != null && !a.dataSaida!.isBefore(period.start) && !a.dataSaida!.isAfter(period.end)).toList();
    return (deadAnimals.length / animalsAtStart.length) * 100;
  }

  double? _calculateBirthRate(DateTimeRange period) {
    final eligibleFemales = herd.where((a) => a.sexo == 'Fêmea' && a.status == AnimalStatus.ativo && period.start.difference(a.dataNascimento).inDays > 450).toList();
    if (eligibleFemales.isEmpty) return 0.0;
    final calvesBorn = herd.where((a) => !a.dataNascimento.isBefore(period.start) && !a.dataNascimento.isAfter(period.end)).toList();
    return (calvesBorn.length / eligibleFemales.length) * 100;
  }

  double? _calculatePregnancyRate(DateTimeRange period) {
    final exposedFemales = herd.where((a) => a.sexo == 'Fêmea' && a.historicoReprodutivo.any((e) => (e.eventType == 'Inseminação' || e.eventType == 'Cio') && !e.date.isBefore(period.start) && !e.date.isAfter(period.end))).toList();
    if (exposedFemales.isEmpty) return 0.0;
    final pregnantFemales = exposedFemales.where((a) => a.historicoReprodutivo.any((e) => e.eventType == 'Diagnóstico de Toque' && e.result == 'Positivo' && !e.date.isBefore(period.start) && !e.date.isAfter(period.end))).toList();
    return (pregnantFemales.length / exposedFemales.length) * 100;
  }

  double? _calculateWeaningRate(DateTimeRange period) {
    final calvesBorn = herd.where((a) => !a.dataNascimento.isBefore(period.start) && !a.dataNascimento.isAfter(period.end)).toList();
    if (calvesBorn.isEmpty) return 0.0;
    final weanedCalves = calvesBorn.where((a) => a.isDesmamado && a.dataDesmame != null && !a.dataDesmame!.isAfter(period.end)).toList();
  ...
  double? get averageAgeAtFirstCalving {
    ...
    final females = herd.where((a) => a.sexo == 'Fêmea');
    for (final female in females) {
      final firstCalving = female.historicoReprodutivo.where((e) => e.eventType == 'Parto').sortedBy((e) => e.date).firstOrNull;
      if (firstCalving != null) {
        final ageInDays = firstCalving.date.difference(female.dataNascimento).inDays;
        ages.add(ageInDays);
      }
    }
