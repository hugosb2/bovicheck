...
class BancoDadosServico {
  ...
  Future<void> _createDB(Database db, int version) async {
    // 1. Propriedades (CORRIGIDO: Adicionado gpsLat e gpsLong)
    await db.execute('''
  ...
  Future<void> adicionarPropriedade(Propriedade p) async {
    ...
    await db.insert('propriedades', p.toMap(),
  ...
  Future<List<Animal>> getAnimaisPorFazenda(String fazendaId) async {
    final db = await database;
    final result = await db.query('animais',
        where: 'fazendaId = ?', whereArgs: [fazendaId], orderBy: 'brinco ASC');
  ...
  Future<void> salvarPesagem(Pesagem pesagem) async {
    final db = await database;
    await db.insert('pesagens', pesagem.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
    await db.update('animais', {'pesoAtualKg': pesagem.pesoKg},
        where: 'id = ?', whereArgs: [pesagem.animalId]);
  }

  Future<List<Pesagem>> getPesagensPorAnimal(String animalId) async {
    final db = await database;
    final res = await db.query('pesagens',
        where: 'animalId = ?', whereArgs: [animalId], orderBy: 'data DESC');
    return res.map((x) => Pesagem.fromMap(x)).toList();
  }

  Future<void> salvarEventoReprodutivo(EventoReprodutivo evento) async {
    final db = await database;
    await db.insert('eventos_reprodutivos', evento.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<List<EventoReprodutivo>> getEventosReprodutivosPorAnimal(
    ...
    final db = await database;
    final res = await db.query('eventos_reprodutivos',
        where: 'animalId = ?', whereArgs: [animalId], orderBy: 'data DESC');
  ...
  Future<void> salvarProducaoLeite(ProducaoLeite evento) async {
    final db = await database;
    await db.insert('producao_leite', evento.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<List<ProducaoLeite>> getProducaoLeitePorAnimal(String animalId) async {
    final db = await database;
    final res = await db.query('producao_leite',
        where: 'animalId = ?', whereArgs: [animalId], orderBy: 'data DESC');
  ...
  Future<void> salvarEventoSanitario(Map<String, dynamic> evento) async {
    final db = await database;
    await db.insert('eventos_sanitarios', evento,
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
