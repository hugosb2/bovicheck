// lib/views/animal/tabs/production_tab.dart
...
class ProductionTab extends StatelessWidget {
  ...
  void _showRecordOptions(BuildContext context, AnimalDetailController controller, MilkRecord record) {
    showModalBottomSheet(context: context, builder: (ctx) => Wrap(children: [
      ListTile(leading: const Icon(Icons.edit_outlined), title: const Text('Editar'), onTap: () {
        Navigator.pop(ctx);
        _showAddMilkRecordDialog(context, controller, recordToEdit: record);
      }),
  ...
  void _showDeleteConfirmation(BuildContext context, AnimalDetailController controller, MilkRecord record) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Confirmar Exclusão'),
      content: const Text('Tem certeza que deseja apagar este registro de produção?'),
      actions: [
  ...
  void _showAddMilkRecordDialog(BuildContext context, AnimalDetailController controller, {MilkRecord? recordToEdit}) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, builder: (ctx) => Padding(
      ...
      child: Form(key: formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
        Text(isEditing ? 'Editar Registro de Leite' : 'Adicionar Registro de Leite', style: Theme.of(context).textTheme.titleLarge),
        ...
        // CORRIGIDO: 'TextInputype' para 'TextInputType'
        TextFormField(initialValue: morningProd?.toString(), decoration: const InputDecoration(labelText: 'Produção da Manhã (L)'), keyboardType: const TextInputType.numberWithOptions(decimal: true), validator: (v) => v!.isEmpty ? 'Obrigatório' : null, onSaved: (v) => morningProd = double.tryParse(v!)),
        TextFormField(initialValue: afternoonProd?.toString(), decoration: const InputDecoration(labelText: 'Produção da Tarde (L)'), keyboardType: const TextInputType.numberWithOptions(decimal: true), validator: (v) => v!.isEmpty ? 'Obrigatório' : null, onSaved: (v) => afternoonProd = double.tryParse(v!)),
        const SizedBox(height: 24),
        ElevatedButton(child: const Text('Salvar'), onPressed: () {
          if (formKey.currentState!.validate()) {
            formKey.currentState!.save();
            final record = MilkRecord(id: recordToEdit?.id ?? const Uuid().v4(), date: selectedDate, morningProduction: morningProd!, afternoonProduction: afternoonProd!);
            if (isEditing) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: productionHistory.isEmpty ? const Center(child: Text('Nenhum registro de produção encontrado.')) : ListView.builder(
        itemCount: productionHistory.length,
        itemBuilder: (context, index) {
          final record = productionHistory[index];
          return ListTile(
            leading: const Icon(Icons.opacity_outlined),
            title: Text('Total: ${record.totalProduction.toStringAsFixed(2)} Litros'),
            subtitle: Text(DateFormat('dd/MM/yyyy').format(record.date)),
            trailing: Text('Manhã: ${record.morningProduction} L\nTarde: ${record.afternoonProduction} L'),
