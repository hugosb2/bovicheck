// lib/views/animal/tabs/weights_tab.dart
...
class WeightsTab extends StatelessWidget {
  ...
  void _showRecordOptions(BuildContext context, AnimalDetailController controller, WeightRecord record) {
    showModalBottomSheet(context: context, builder: (ctx) => Wrap(children: [
      ListTile(leading: const Icon(Icons.edit_outlined), title: const Text('Editar'), onTap: () {
        Navigator.pop(ctx);
        _showAddWeightDialog(context, controller, recordToEdit: record);
      }),
  ...
  void _showDeleteConfirmation(BuildContext context, AnimalDetailController controller, WeightRecord record) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Confirmar Exclusão'),
      content: const Text('Tem certeza que deseja apagar este registro de peso?'),
      actions: [
  ...
  void _showAddWeightDialog(BuildContext context, AnimalDetailController controller, {WeightRecord? recordToEdit}) {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, builder: (ctx) => Padding(
      ...
      child: Form(key: formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
        Text(isEditing ? 'Editar Pesagem' : 'Adicionar Nova Pesagem', style: Theme.of(context).textTheme.titleLarge),
        ...
        TextFormField(initialValue: weight?.toString(), decoration: const InputDecoration(labelText: 'Peso (kg)'), keyboardType: const TextInputType.numberWithOptions(decimal: true), validator: (v) => v!.isEmpty ? 'Obrigatório' : null, onSaved: (v) => weight = double.tryParse(v!)),
        const SizedBox(height: 24),
        ElevatedButton(child: const Text('Salvar'), onPressed: () {
          if (formKey.currentState!.validate()) {
            formKey.currentState!.save();
            final record = WeightRecord(id: recordToEdit?.id ?? const Uuid().v4(), date: selectedDate, weight: weight!);
            if (isEditing) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: weightHistory.isEmpty ? const Center(child: Text('Nenhum registro de peso encontrado.')) : ListView.builder(
